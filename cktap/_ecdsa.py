...
def decode_privkey(priv, formt=None):
    if formt == 'decimal':
        ...
        return decode_base256(priv)
    elif formt == 'bin_compressed':
        return decode_base256(priv[:32])
    raise ValueError("Invalid format!")
...
def privkey_to_pubkey(privkey: bytes):
    ...
    return encode_pubkey(fast_multiply(G, privkey), "bin_compressed")
...
def ecdh(pubkey: bytes, privkey: bytes) -> Tuple[int, int]:
    pubkey = decode_pubkey(pubkey, "bin_compressed")
    privkey = decode_privkey(privkey, "bin_compressed")
    # http://safecurves.cr.yp.to/twist.html
    if not isinf(pubkey) and (pubkey[0]**3+B-pubkey[1]*pubkey[1]) % P != 0:
        raise ValueError("Point not on curve")
    return hashlib.sha256(encode_pubkey(fast_multiply(pubkey, privkey), "bin_compressed")).digest()